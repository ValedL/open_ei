defmodule OpenElixirIntelligenceWeb.OpenEAIPortal do
  ...
  def mount(_params, _session, socket) do
    ...
    # Phoenix.PubSub.subscribe(OpenElixirIntelligence.PubSub, OpenElixirIntelligence.OpenEI.topic_response_stream())
    # Phoenix.PubSub.subscribe(OpenElixirIntelligence.PubSub, OpenElixirIntelligence.OpenEI.topic_user_message())
    Phoenix.PubSub.subscribe(
      OpenElixirIntelligence.PubSub,
      OpenElixirIntelligence.OpenEAI.topic_response_stream()
    )
    ...
    {:ok,
     assign(socket,
       text: nil,
       status: OpenElixirIntelligence.OpenEAI.get_agent_status(),
       is_agent_on: OpenElixirIntelligence.OpenEAI.is_agent_on(),
       content: "",
       raw_messages: OpenElixirIntelligence.OpenEAI.get_raw_messages()
     )}
  end
  ...
  def handle_event("new_chat", _params, socket) do
    OpenElixirIntelligence.OpenEI.reset_state()

    {:noreply,
     assign(socket,
       content: "",
       raw_messages: OpenElixirIntelligence.OpenEI.get_raw_messages()
     )}
  end

  def handle_event("cancel_generation", _params, socket) do
    OpenElixirIntelligence.OpenEI.cancel_generation()

    {:noreply, socket}
  end

  def handle_event("toggle_eai", _params, socket) do
    status = OpenElixirIntelligence.OpenEAI.toggle_state()
    is_agent_on = OpenElixirIntelligence.OpenEAI.is_agent_on()
  ...
  def handle_event("generate_exception", _params, socket) do
    Phoenix.PubSub.local_broadcast(
      OpenElixirIntelligence.PubSub,
      OpenElixirIntelligence.VeryBadCode.topic(),
      {:divide_by_zero, ""}
    )
  ...
  def render(assigns) do
    ~H"""
    <div class="container mt-4 w-full pb-32 flex">
      <!-- Added padding-bottom to make space for the floating form -->
      <div class="w-1/4 flex flex-col items-center fixed" style="top: 10%; left: 5%;">
        ...
        <.button
          ...
          class="top-80 m-4 z-5"
        />
        <.button
          color="warning"
          label="Generate Exception"
          variant="shadow"
          phx-click="generate_exception"
          class="top-75 m-4 z-1"
        />
        <div class="mt-2 bg-white shadow-lg rounded-lg p-4">
          <label
            class="text-lg font-semibold text-gray-700 dark:text-gray-200 mr-4"
            style="color: black; font-family: 'Helvetica Neue', sans-serif;"
          >
            Self-Healing Agent:
          </label>
          <.button
            phx-click="toggle_eai"
            class="mt-2"
            color="success"
            label={@status}
            variant={"#{if @is_agent_on, do: "shadow", else: "outline"}"}
          />
        </div>
      </div>
      <div class="w-2/3 mx-auto" style="margin-left: 30%;">
        <!-- Shifted the card div to the right -->
        ...
        <% end %>
        <div class="mb-20"></div>
        <!-- Added padding below the last card -->
        <div
          class="fixed bottom-0 z-50 w-1/2 mx-auto bg-white dark:bg-gray-800 shadow"
          style="left: 50%; transform: translateX(-50%); background: rgba(255, 255, 255, 0.6);"
        >
